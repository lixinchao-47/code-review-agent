"""
风格审查员优化测试 #01：快速冒烟 —— prompt 文本 + 模型校验 + 图结构（无 LLM）

用法：python tests/optimization/style_reviewer/opt_style_01_smoke.py
"""
import sys
import warnings
...
if __name__ == "__main__":
    ...
    ok = (issue.severity == Severity.MEDIUM
          ...
          and issue.pep8_ref is None)
    total += 1; passed += 1 if ok else 0
    print(f"  {'✅' if ok else '❌'} Issue: 非法枚举→MEDIUM/OTHER, null→''/0")

    rr = ReviewResult(dimension="style", issues=None)
    ok = rr.issues == []
    total += 1; passed += 1 if ok else 0
    print(f"  {'✅' if ok else '❌'} ReviewResult: null issues→[]")

    rr2 = ReviewResult(dimension="style", issues=[issue])
    ok = rr2.dimension == ReviewDimension.STYLE and len(rr2.issues) == 1
    total += 1; passed += 1 if ok else 0
    print(f"  {'✅' if ok else '❌'} ReviewResult: dimension=style + issues 链路通畅")
